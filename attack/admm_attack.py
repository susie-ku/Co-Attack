...
class MirrorDescentAttack:
    ...
    def loss(self, image, adv_image, W, p):
        I = torch.eye(p.shape[0])
        l1 = self.criterion(adv_image, torch.matmul(self.V, self.W0))
        l2 = self.criterion(image, torch.matmul(self.V, torch.matmul(self.W0, I - p * I) + torch.matmul(W, p * I)))
        return 0.5 * l1 + 0.5 * l2

    def normalize(self, x):
        return x / torch.norm(x)

    def txt_step(self, W, p):
        p_ = self.normalize(p - self.lr2 * p.grad)
        W_ = self.normalize(W * torch.exp(-self.lr1 * W.grad))
        print(p_)
        print(p)
        print(p.grad)
        return W_, p_

    def img_step(self, image):
        image_ = image + self.alpha * image.grad.sign()
        eta = torch.clamp(image_ - image, min = -self.eps, max = self.eps)
        image_ = torch.clamp(image + eta, min=0, max=1) #.detach_()
        return image_

    def truncate(self, x):
        indices = self.zero_indices(x.abs(), self.k)
        x = x.scatter_(index=indices, value=0.)
        return x

    def attack(self):
        W = torch.zeros(self.W0.shape)
        indices = torch.randint(0, W.shape[1], (W.shape[0],))
        W[torch.arange(W.shape[0]), indices] = 1
        W = W.float()
        W = W.requires_grad_()
        p = torch.zeros(W.shape[1])
        p[:self.k] = 1.
        p = p.requires_grad_()
        image = self.image.clone().requires_grad_()
        for i in range(self.num_iters):
            loss = self.loss(self.image, image, W, p)
            loss.backward()
            W, p = self.txt_step(W, p)
            image = self.img_step(image)
        return image, W, p
